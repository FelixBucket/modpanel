...
class InfractionForm(ModelForm):
    ...
    def clean(self):
        ...
        if not cleaned_data.get('change_level', False) and not cleaned_data.get('speedchat_only', False) and not cleaned_data.get('no_true_friends', False) and not cleaned_data.get('no_community_areas', False):
